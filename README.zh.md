...
Notion 官方 MCP 服务器只支持带 PKCE 的 OAuth 2.0——没有静态 API key 的接入方式，而 DeepSeek Harness 本身也没有为 MCP 服务器提供可复用的 OAuth 能力。`dsh-notion` 补上了这一环：它替你跑完整套授权流程，把 token 通过 dsh 的凭据层安全落盘，在后台静默刷新保持有效，并把官方 Notion MCP 客户端挂载进来，让工具以 `mcp__notion__*` 的形式出现。
